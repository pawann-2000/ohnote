...
import { toGitHubBlobUrl, toRawUrl, toHuggingFaceRawUrl, encodePath } from "./parser.js";
...
export const PLATFORMS = [
  ...
  },
  {
    ...
    build: (info, sourceUrl) => {
      if (!info) return null;
      const target = toGitHubBlobUrl(info) ?? toRawUrl(info) ?? sourceUrl;
  ...
  },
  {
    ...
    build: (info) => {
      ...
      if (info?.source === "huggingface")
        // nbviewer renders any https .ipynb URL via /urls/<host/path>.
        return "https://nbviewer.org/urls/" + toHuggingFaceRawUrl(info).replace(/^https:\/\//, "");
      return null;
    },
  },
  {
